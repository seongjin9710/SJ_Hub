...
$(document).ready(function(){
	...
	majorApplication(stuNum);
	//대학 select -> 각 대학에 맞는 전공 select 띄우기
	$(document).on('change', '#collegeC', function() {
	...
	//취소 버튼
	$(document).on('click', '.majorBtn', function() {
		var stuNum = $('#stuNum').val();
		deleteMajorApplication(stuNum);
...
(function($){
    ...
    //신청 내역
	majorApplication = function(stuNum){
		$.ajax({
		      url: 'majorApplicationList.do', //요청경로
		      type: 'post',
		      ...
		      success: function(result) { // ajax 통신 성공 시 실행부분. result가 결과 데이터를 가진다.
		    	  ...
		    	  var str = '';
		    	    ...
		    	    str +=`</table>`;
		    	    $(result).each(function(index, element){
			    	    if(element.majorStatus == 'N' && element.applyType == 1) {
			    	    	str +=`<input type="button" value="취소" class="majorBtn">`;			    	    	
			    	    }
			    	    else if(element.applyType == 1 && element.majorStatus == 'Y') {
	...
	deleteMajorApplication = function(stuNum){
  	  ...
  	  if(isConfirm)	{
  		  ...
  		  $.ajax({
  			  url: 'deleteMajorList.do', //요청경로
  			  type: 'post',
  			  data: {'stuNum':stuNum}, //요청경로로 던질 파라메터. '파레메터명':파라메터
  			  success: function(result) { // ajax 통신 성공 시 실행부분. result가 결과 데이터를 가진다.
  				  				  
  					  location.href = 'majorApplication.do?mainNum=' + mainNum + '&sideNum=' + sideNum + '&sideDetailNum=' + sideDetailNum; 
  					  alert('취소되었습니다.');
